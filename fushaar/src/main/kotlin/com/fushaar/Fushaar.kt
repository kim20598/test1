...
class Fushaar : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        val items = doc.select("div.post a").mapNotNull {
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        return doc.select("div.post a").mapNotNull {
            val href = it.attr("href").toAbsolute()
            val title = it.selectFirst("h2, h3")?.text() ?: return@mapNotNull null
            val poster = it.selectFirst("img")?.attr("src")?.toAbsolute()
            newMovieSearchResponse(title, href, TvType.Movie) {
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val poster = doc.selectFirst("div.post img")?.attr("src")?.toAbsolute()
    ...
    override suspend fun loadLinks(
        ...
        val links = doc.select("a[href*=\"watch\"], a[href*=\"download\"]").mapNotNull {
            it.attr("href").toAbsolute()
        }
