...
class Akwam : MainAPI() {
    ...
    override suspend fun loadLinks(
        ...
        // Simple scraping - look for download buttons first
        document.select("a:contains(تحميل), a.btn-download").forEach { downloadBtn ->
            val downloadUrl = downloadBtn.attr("href").toAbsolute()
            if (downloadUrl.isNotBlank()) {
                loadExtractor(downloadUrl, data, subtitleCallback, callback)
            }
        }
        
        // Then look for watch buttons
